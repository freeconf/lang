...
class Device():
    ...
    def __init__(self, driver, yangPath, hnd_id=None):
        self.driver = driver
        if not hnd_id:
            req = pb.fc_g_pb2.NewDeviceRequest(yangPath=yangPath)
    ...
    def add_browser(self, browser):
        req = pb.fc_g_pb2.DeviceAddBrowserRequest(deviceHnd=self.hnd.id, browserHnd=browser.hnd.id)
        self.driver.g_device.DeviceAddBrowser(req)

    def get_browser(self, moduleIdent):
        req = pb.fc_g_pb2.DeviceGetBrowserRequest(deviceHnd=self.hnd.id, moduleIdent=moduleIdent)
    ...
    def apply_startup_config(self, configFile):
        req = pb.fc_g_pb2.ApplyStartupConfigRequest(deviceHnd=self.hnd.id, configFile=configFile)
